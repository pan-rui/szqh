package com.yjf.yrd.front.controller.check;
...
import com.yjf.yrd.dataobject.UserRelationDO;
...
import com.yjf.yrd.user.query.order.MailOrMobileOrder;
...
public class BaseCheckController extends BaseAutowiredController {
	...
	public Object checkCertNoByType(String certNo) throws Exception {
		...
		if (bool) {
			...
			if (StringUtil.isNotEmpty(checkType)) {
				long count = userQueryService.countUserCertNoAndRole(certNo, checkType,userId);
				if (count > 0) {
					map.put("code", 0);
	...
	public Object sendSmsCode(String mobile, String business, String extendMessage)
																					throws Exception {
		logger.info("发生手机验证码，入参[{}],[{}]", mobile, business);
		Map<String, Object> map = new HashMap<String, Object>();
		if (SessionLocalManager.getSessionLocal().getUserBaseId() != null) {
			UserInfo baseUser = userQueryService.queryByUserBaseId(
	...
	@ResponseBody
	@RequestMapping("sendSmsCodeModifyPhone")
	...
	@ResponseBody
	@RequestMapping("sendSmsCodeForgetPassword")
	public Object sendSmsCodeForgetPassword(HttpServletRequest request, String mobile,
		...
		YrdBaseResult baseResult = smsManagerService.sendSmsCode(userBaseInfo.getMobile(),
			SmsBizType.getByCode(business));
		if (baseResult.isSuccess()) {
			map.put("code", 1);
			map.put("message", "发生手机验证码成功");
	...
	public Object sendSmsPasswordCode(String mobile, String business, String userName)
		...
		YrdBaseResult yrdBaseResult = smsManagerService.sendSmsCode(mobile,
			SmsBizType.getByCode(business));
	...
	public Object checkUserNameAndType(String userName, String type) throws Exception {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		YrdBaseResult userBaseReturnEnum = userBaseInfoManager.validationUserName(userName);
...
/*	*//**
	...
	public JSONObject validationBroker(String referees) {
		...
		UserRelationInfo relationInfo = userRelationQueryService.findUserRelationByMemberNo(
			referees).getQueryUserRelationInfo();
		
		if (relationInfo != null) {
			long refereeId = relationInfo.getChildId();
			...
			if (isBroker) {
				parentId = relationInfo.getChildId(); // 推荐人是经纪人
				json.put("code", 1);
