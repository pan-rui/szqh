...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="PageList">
     ...
     <xsl:template match="TableLetter"> 
      ...
      <fo:block  wrap-option="wrap" language="zh" text-align="left" text-indent="20px"  space-before="8pt" space-after="2pt"  line-height="17pt">一、<xsl:value-of select="ENTERPRISE"/>承诺对出借人的资金本金及收益承担连带担保责任；</fo:block>
      ...
      <fo:block  wrap-option="wrap" language="zh" text-align="left" text-indent="20px"  space-before="8pt" space-after="2pt"  line-height="17pt">被担保的金额为：人民币 <xsl:value-of select="DMONEY"/> 万元，年化收益率为 <xsl:value-of select="PER"/>  %。</fo:block>
      ...
      <fo:block  wrap-option="wrap" language="zh" text-align="left" text-indent="20px"  space-before="8pt" space-after="2pt"  line-height="17pt">如借款人不能按照规定在到期日足额偿还相应本金及收益，担保人在收到《履责通知书》后于三个工作日将代偿本金及收益足额划入担保人在公司平台关联的银行账户中。并完成全部代偿相关手续。 </fo:block>
      ...
      <fo:block  wrap-option="wrap" language="zh" text-align="left" text-indent="20px"  space-before="8pt" space-after="2pt"  line-height="17pt">担保人保证范围包括借款的本金及收益，合同编号为：<xsl:value-of select="CONTRACTNO"/></fo:block>
       ...
       <!-- 定义表头 //-->
	     ...
	     <fo:table-row height="20pt">
	      ...
	      <fo:table-row >
	         ...
	         <fo:table-cell  >
	             <fo:block text-align="right"  wrap-option="wrap" ><xsl:value-of select="NYYYY"/>年<xsl:value-of select="NMM"/> 月<xsl:value-of select="NDD"/> 日</fo:block>
	         </fo:table-cell>
	     </fo:table-row>
