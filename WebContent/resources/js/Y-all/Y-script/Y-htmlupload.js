...
define(function(require, exports, module){
	...
	Y.inherit('HtmlUploadify', 'Window', {
		...
		clearValue: function(item){
			...
			var newObj = $('<input type="file" style="font-size:14px;float:left;" name="'+fileObjName+'"/>');
			obj.after(newObj);
			obj.remove();
		},
		...
		addItem:function(){
			...
			div.append($('<input type="file" name="'+fileObjName+'" style="font-size:14px;float:left;" />'));
			div.append("<span class='fn-upinfo' style='display:none;margin-left:10px;color:#999999;font-size:12px;'></span>");
	...
	Y.HtmlUploadify.defaults = $.extend({}, Y.Window.defaults, {
		...
		addAble: false,//可添加新的上传
		removeAbled: false//可删除不需要的上传位
	});	
});
