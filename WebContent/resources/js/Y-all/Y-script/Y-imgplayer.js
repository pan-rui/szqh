...
define(function(require, exports, module){
    ...
    Y.inherit('ImgPlayer','Window',{
		addImg:function(ele){
			...
			$('body').delegate(ele,'click',function(){
				...
				_this.ele = this;
				cfg.index = $(_this.eleArr).index(this);
				_this.loadHandler();
			});
		},
	    ...
	    doInit: function(cfg){
			...
			if(!cfg.mainDiv){
				$('body').append(cfg.mainDiv = $('<div class="photo-browse" style="display:none;"><div class="main-photo"></div><div class="mod-area" align="center"><p class="path-title">名称</p><p class="info-title">名称</p><a href="javascript:;" class="magnify"></a><a href="javascript:;" class="lessen"></a><a href="javascript:;" class="rota-l"></a><a href="javascript:;" class="rota-r"></a><a href="javascript:;" class="full"></a></div><a href="javascript:;" class="prev-photo">上一张</a><a href="javascript:;" class="next-photo">下一张</a><a href="javascript:;" class="close-photo close">关闭</a></div>'));
			}
		...
		transformHandler:function(num){// num 转的角度 90 -90
			this.doTransform(num);
			this.fire('transformHandler');
			return this;
		},
		...
		zoomHandler:function(num){// num是缩放的 比例 0.1放大 -0.1缩小
			this.doZoom(num);
			this.fire('zoomHandler');
			return this;
		},
		doZoom:function(num){
			var img = this.itemImg;
		...
		gotoShowHandler:function(num){// num 可为'+1'字符串 向后1张, -1 向前1张, 1跳到第一张
			this.doGotoShow(num);
			this.fire('gotoShowHandler');
			return this;
		},
		...
		setEle:function(ele){//设置要显示的img 或者要跳转的位置
			var cfg = this.cfg;
			var _this = this;
			if(typeof ele != "object"){
				this.gotoShowHandler(ele);
			}else{
				var arr = $(cfg.eleArr);
				var img = this.itemImg;
		...
		loadHandler:function(){//初始化
			this.doLoad();
			this.fire('loadHandler');
			return this;
		},
		doLoad:function(){
			var img = this.itemImg;
		...
		doMoveCenter:function(sts, ele, speed){
			...
			function load(){
				...
				var showAfter = _this.cfg.showAfter;
				_this.reWndSize(sts,ele);
				if(showAfter){
					if(typeof showAfter == 'function'){
						showAfter(img, $(cfg.mainDiv));
					}else if(typeof showAfter == 'string'){
						if(showAfter && typeof _this[showAfter] == 'function'){
							_this[showAfter](img,$(cfg.mainDiv));
						}
					}
				}
			}
	...
	Y.ImgPlayer.defaults = $.extend({}, Y.Window.defaults, {
		...
		showAfter: null//显示完成后的回调函数 可配置函数 或者字符串 字符串为内置函数
	});	
});
