define(function(require) {
	...
	$('.modify').click(function() {
		...
		changeWnd = $('body').window({
			...
			closeEle : '.cancel',
			show : function() {
				this.wnd.find('input').not('[name=type],[name=md5UserBaseId],[name=userBaseId]').val('');
			}
		});
	});
	$('.modifyYjfPass').click(function() {
		$('b.error-tip').hide();
		changeWnd = $('body').window({
			content : '#n-fn-layer-yjfpassword',
			simple : true,
			closeEle : '.cancel',
			show : function() {
				this.wnd.find('input').not('[name=type],[name=md5UserBaseId],[name=userBaseId]').val('');
	...
	$("#forgotPayPwd").on("click", function(){
		$.ajax({
    ...
    var boundUpdateMobileSecondWnd;
		...
		updateBoundMobile_form.validate({
			...
			submitHandler : function() {
				...
				updateBoundMobile_form.ajaxSubmit({
					success : function(res) {
						if (res.code == 1) {
                            boundUpdateMobileSecondWnd = $('body').window({
                                content : '#bound_updateMobile_second',
                                simple : true,
                                closeEle : '.cancel',
                                show : function() {
                                    this.wnd.find('input[name=newMobile],input[name=code]').val('');
						...
						if (res.code == 0) {
							$('body').window({
								...
								closeEle : '.cancel',
                                show:function(){
                                    this.wnd.find('#boundMobile_no_txt').html(res.message);
                                }
    ...
    var updateBoundMobile_form_second = $('#updateBoundMobile_form_second');
    if (updateBoundMobile_form_second.length) {
        updateBoundMobile_form_second.validate({
            errorClass : 'error-tip',
            errorElement : 'b',
            errorPlacement : function(error, element) {
                if (element.attr('name') == 'mobile') {
            ...
            submitHandler : function() {
                boundUpdateMobileSecondWnd.close();
                updateBoundMobile_form_second.ajaxSubmit({
                    success : function(res) {
                        if (res.code == 1) {
                            $('body').window({
                                content : '#boundMobile_ok',
    ...
    $('#getCode30').click(function() {
        var business = $("#business30").val();
        var mobile = $("#newMobile").val();
        var countdown = Y.getCmp('getCode30');
        if (!$("#newMobile").valid()) {
            countdown.close();
            return;
        } else {
            sendSmsCodeModifyPhone(business, mobile, countdown);
        }
    });
    ...
    function sendSmsCodeModifyPhone(business, mobile, conutdown) {
        $.ajax({
            url : '/anon/sendSmsCodeModifyPhone?date='+new Date().getTime(),
            dataType : 'json',
            data : {
                mobile : mobile,
                business : business
		...
		bound_updateMail_form.validate({
			...
			submitHandler : function() {
				...
				bound_updateMail_form.ajaxSubmit({
					success : function(res) {
						if (res.code == 1) {
							window.location.href="/userManage/accountSetting";   //账户设置
							alert('修改邮箱成功');
