function getId(replanId){
	var data = {replanId:replanId};
	 var t = $('#ih');
	 var f = $('#f');
	 var fd = $('#fd');	
	 t.empty();
	 ...
	 fd.empty();
	$.ajax({
		url:'/tradeQuery/getRepayPlanId/'+replanId,
		type:'post',
		dataType:'json',
		async:false,
...
//				backUrl = backUrl;
				document.location.href ='/tradeQuery/repay/12/1';
			}else{
				alert(result.message);
    ...
    window.confirmClick=function(paytk){
        ...
        if('1' == result.code){
            alert(result.message);
            window.location.href = "/tradeQuery/repay/12/1";
        }else{
            alert(result.message);
            window.location.href = "/tradeQuery/repay/12/1";
        }
